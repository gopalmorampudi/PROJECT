...
<html>
  <head>
	...
	<body class="skin-blue sidebar-mini" onload="checkFlag();">
	
	<input type="hidden" id="flag" value="<?=(isset($_SESSION['msg']))?$_SESSION['msg']:''?>"/>
		<div class="wrapper">
...
<?php
 $sql="select ts.id,t.tutor_id,sub.subject_id,t.tutor_name,s.student_name,sub.subject_name,ts.date_selected,ts.time_selected,ts.student_email from tutor t,student s,tutor_student_classes ts,subject sub where t.tutor_id=ts.tutor_id and s.emailid=ts.student_email and sub.subject_id=ts.subject_id and ts.`status`='pending'";
...
<td><?=$res['date_selected']?></td>
<td><?=$res['time_selected']?></td>
<td>
	<a href="insert_data1.php?action=check_schedule&schedule_id=<?=$res['id']?>&tutor_id=<?=$res['tutor_id']?>&subject_id=<?=$res['subject_id']?>&date=<?=$res['date_selected']?>&time=<?=$res['time_selected']?>" class="btn btn-primary">Accept</a>
	<a href="insert_data1.php?action=reject_student_course_request&schedule_id=<?=$res['id']?>" class="btn btn-primary">Reject</a>
</td>
</tr>
<?php
}
