...
$checkquery="SELECT * from tutor where password='$oldpassword' and emailid='$tutor_email'";

$checkresults=mysql_query($checkquery);
if(mysql_num_rows($checkresults)>0){
	$updatequery="UPDATE tutor SET password='$confpassword' where password='$oldpassword' and emailid='$tutor_email'";
	$updateres=mysql_query($updatequery);
	header('Location: changepass.php?messg=success');
}else{
	header('Location: changepass.php?messg=fail');
}
}
 */
...
$admin_user=$_SESSION['adminuser'];
 ...
 echo $complaint_id=$_REQUEST['complaint_id'];
echo $emailid=$_REQUEST['emailid'];
echo $subject_name=$_REQUEST['subject_name'];
echo $tutor_name=$_REQUEST['tutor_name'];
echo $description=$_REQUEST['description'];
echo $reply=$_REQUEST['reply'];
 ...
 echo $sql="update complaint set reply='$reply',replied_by='$admin_user',status='Replied' where complaint_id='$complaint_id'";
$res=mysql_query($sql);
if($res)
{
$br=<<<BR
 \r\n\r\n\r\n
BR;
echo $new='Subject Name:-'.$subject_name.$br .$emailid.$br.'Tutor Name:-'.$tutor_name.$br.'Description:-'.$description.$br.'Reply:-'.$reply.$br.'Replied by:-'.$admin_user.$br;
...
$message->setFrom(array('nukesh.smts@gmail.com'=>"Enquiry"));
