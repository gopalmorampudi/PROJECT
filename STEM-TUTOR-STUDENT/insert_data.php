...
if($action=='insert_tutor_reg'){
	...
	$sql="insert into tutor(tutor_name,emailid,password,phone_number,address,subject,working_hours,working_days) values('$tutor_name','$emailid','$password','$phone_number','$address','$sub','$working_hours','$working_days')";
	$res=mysql_query($sql);
	if($res){
		 //echo "sucesss";
		  header('Location: login.html?mesg=success');
			}else{
		//echo "fail";
		header('Location: login.html?mesg=failed');
	}
}
...
if($action=='update_student'){
	...
	if($res){
		...
		//echo 'Failure';
		header('Location: editstudent_profile.php?mesg=fail');
	}
}
